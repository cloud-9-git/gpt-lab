...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        # dropout: attention 계산 과정에서 regularization 적용
    ...
    def forward(
        ...
        # 한 번의 projection으로 처리하는 게 구현이 단순할 듯 합니다.
        ...
        # scaling을 하지 않으면 값이 너무 커져 softmax가 불안정(overflow)해질 수 있기 때문.
        # softmax는 지수함수 기반이니깐, 값이 너무 커지면 값이 기하급수적으로 커지기에 overflow가 발생할 수 있음.

        # 5. causal mask를 적용한다.
        # causal_mask=True이면 미래 토큰 위치를 보지 못하게 mask 처리한다.
        # (T, T) upper-triangular mask를 만들고, 가릴 위치를 -inf로 채운다.
